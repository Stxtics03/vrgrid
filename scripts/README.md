...
| `timing_table.py` | The Day-6 per-stage latency table: p50/p99/max and headroom at p99, machine named, `--alloc` for transient bytes per frame. **`--seq 08` times the WHOLE frame on real data** -- all twelve stages, perception included, one Timer shared by `iter_pipeline` and `MapEngine`. Drives the real lattice, scatter, fuse, cleanup, pyramid and shift off a synthetic sweep. Covers the mapping back end only, and prints the unmeasured stages with their owner rather than dropping them |
...
| `ablation_table.py` | Schedule ablation: cells, memory and per-stage latency per schedule against uniform-grid baselines, under ONE frozen threshold set whose sha256 is printed and re-checked. `--expect-thresholds` asserts it |
...
Every script on the Gate 6 list now exists.
