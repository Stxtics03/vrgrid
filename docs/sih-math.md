...
>
> ***⛑ The sign of the correction is not settled, and it is scene-dependent.*** *Every before → after number here is the synthetic sequence, where dropping the stale cells* lowers *RMSE — the stale value was written at grazing incidence on the 6% ramp and is worse than the live annulus. A second measurement reported 3 Sep against seq 07/08 found the opposite: RMSE understated by 3–12% across rings 1–3, so dropping the stale cells* raises *it. Both are plausible, because which population is the harder ground is a property of the scene — on real urban data ring 2's stale interior is road the vehicle has driven over, flatter than its live 25–50 m annulus of verges and facades. That measurement could not be reproduced here (no data root on this machine, no `M*` artefact for either sequence), so* **the direction on real data is unverified and no report sentence may claim this correction improves our numbers.** *What is settled is the mechanism, the population size, and the asymmetry across schedules. Re-measure on 07/08 the moment `M*` exists.*
>
