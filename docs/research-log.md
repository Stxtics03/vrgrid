...
Re-measured, `--frames 14`: R(S) 5/10/20/40 **0.354 → 2.389**, uniform 10 cm **1.389 → 0.000**, uniform 20/40/80 cm **0.000 → 3.354/3.854/3.854**. The all-zeros are gone and so is the non-monotone spike at uniform 10 cm that Shrestha flagged in `regret_plot.py`; the four uniforms are now identical at 12, 14 and 24 frames. The frozen schedules still move with frame count (2.389, 2.389, 1.450), so that half of his finding stands. Ring 1 RMSE without the transient layer re-measures 0.41 → 12.72 cm. The confound table in `plan_regret.py` no longer reproduces at all — the window is 99.1% common support now — so it is re-measured, kept, and reproducible from `scripts/eval_synthetic.py --confound`.
...
**So what:** One sequence writer, one frame convention, and the real-data path is exercised end to end on a scene with a negative obstacle actually in it. **The uncomfortable part goes in the report, not in a footnote:** on this scene our schedule costs 29.06 MB to score R(S) = 2.389 where uniform 10 cm costs 18.19 MB to score 0.000. That is a synthetic scene whose hazards sit off the planned lane — Shrestha's open diagnosis, and `PLAN_LANE_CELLS` is deliberately untouched — but until the query is posed properly the honest statement is that the money plot does not yet show what it was built to show.
